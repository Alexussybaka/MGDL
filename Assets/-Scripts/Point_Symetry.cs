...
public class Point_Symetry : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (bridge_shape) Debug.DrawLine(new Vector3(vertexes[0].x, 0f, vertexes[0].y), new Vector3(vertexes[vertexes.Count - 1].x, 0f, vertexes[vertexes.Count - 1].y));
        ...
        if (bridge_shape) Debug.DrawLine(new Vector3((2 * point.position.x) - vertexes[0].x, 0f, (2 * point.position.z) - vertexes[0].y), new Vector3((2 * point.position.x) - vertexes[vertexes.Count - 1].x, 0f, (2 * point.position.z) - vertexes[vertexes.Count - 1].y));
    }
}

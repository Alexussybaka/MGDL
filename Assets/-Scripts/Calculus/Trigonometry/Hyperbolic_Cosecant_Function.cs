...
public class Hyperbolic_Cosecant_Function : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        {
            float eval = Mathf.Pow(2 * e, i) / (Mathf.Pow(e, 2 * i) - 1);
    ...
    public void Visualise_Examined_Number()
    {
        if (eulers)
        {
            ...
            evaluation = Mathf.Pow(2 * e, number * e) / (Mathf.Pow(e, 2 * number * e) - 1);
        ...
        {
            // Calculating evaluation for the analyzed number
            evaluation = Mathf.Pow(2 * e, number) / (Mathf.Pow(e, 2 * number) - 1);

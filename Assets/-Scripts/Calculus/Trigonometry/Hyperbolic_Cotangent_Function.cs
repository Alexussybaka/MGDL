...
public class Hyperbolic_Cotangent_Function : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        {
            float eval = (Mathf.Pow(e, 2 * i) + 1) / (Mathf.Pow(e, 2 * i) - 1);
            if (Mathf.Abs(eval) <= limit && i != 0) vectors.Add(new Vector3(i, 0f, eval));
        }
    ...
    public void Visualise_Examined_Number()
    {
        if (eulers)
        {
            ...
            evaluation = (Mathf.Pow(e, 2 * number * e) + 1) / (Mathf.Pow(e, 2 * number * e) - 1);
        ...
        {
            // Calculating evaluation for the analyzed number
            evaluation = (Mathf.Pow(e, 2 * number) + 1) / (Mathf.Pow(e, 2 * number) - 1);
